...
def set_contact():
    ...
    # print(name, phonr_num, email, addr)
...
def load_contacts(list):
    try:
        ...
        f.close()       # 파일이 없어서 생기는 예외는 파일 생성하고 함수아웃
        return
...
def run():
    ...
    # temp = Contact('정재영', '010-4783-7552', 'xstar202@naver.com', '부산시 남구 용소로 45')
    # set_contact()
    clear_console()
    ...
    while True:
        ...
        if sel_menu == 1:           # 연락처 추가
            try:                # 15-2 연락처 잘못 입력했을때 예외처리
               contact = set_contact()
               contacts.append(contact)
               input('주소록 입력 성공')
            except Exception as e:
                print('이름/전화번호/이메일/주소의 형태로 입력해주세요.')
                input()
            finally:
                clear_console()
